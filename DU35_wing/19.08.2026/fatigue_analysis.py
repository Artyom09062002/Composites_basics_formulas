...
STATIONS = ROOT / "results" / "structural" / "glass_station_results.csv"
STIFFNESS = ROOT / "data" / "reference_blade" / "blade_stiffness.csv"
REFERENCE_DAMAGE = ROOT / "data" / "fatigue" / "sandia_reference_damage.csv"
TARGETS = ROOT / "data" / "fatigue" / "nrel_60m_fatigue_targets.csv"
WEB_GEOMETRY = ROOT / "sessions" / "day04_full_blade" / "results" / "cad_web_geometry.csv"
...
# Saertex VU 90079 under R=-1 gives applied axial S=156*N^-0.103 MPa.
# Their Table 2 / ASTM D3518 relation gives ply shear = applied stress / 2,
# hence the measured reference shear curve is S6=78*N^-0.103 MPa.
SAERTEX_VU90079_MEASURED_RM1 = SNCurve(
    "Saertex VU 90079 measured R=-1 shear reference",
    1.0 / 0.103,
    78.0,
    13.1,
    "Mandell, Miller & Samborsky 2013, AIAA-2013-1630, Fig. 7 Fabric L "
    "and Table 2 (ASTM D3518 shear = axial/2)",
)
# The robust design envelope deliberately remains more severe than that
# measured curve: C=62 MPa and b=9.  It is not fitted to make the design pass.
DB_ROBUST_B9 = SNCurve(
    "Saertex DB conservative robust envelope",
    9.0,
    62.0,
    13.1,
    "Estimated conservative envelope: measured Saertex VU 90079 R=-1 "
...
def write_phase7_outputs() -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    ...
    pd.DataFrame([
        ...
        {"curve": SAERTEX_VU90079_MEASURED_RM1.name,
         ...
         "b": SAERTEX_VU90079_MEASURED_RM1.b, "status": "measured reference",
